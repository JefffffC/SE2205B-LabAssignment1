...
public class Assignment1{
  ...
  {
      int resultArray[][] = initMatrix(size);
      if (size == 1) {
          resultArray[0][0] = A[0][0] * B[0][0];
          return resultArray;
      }
      else {
          int nullArray[][] = initMatrix(size);
  ...
  public int[][] readMatrix(String filename, int n) throws Exception
  {
      ...
      File homeFile = new File(workingDir + "Test.java");
      String targetDir = homeFile.getParent();
      File readFile = new File(targetDir + "\\" + filename);
      Scanner sc = new Scanner(readFile);
      int i = 0;
      int scannedArray[][] = new int[n][n];
      while (i < (n*n)) {
          for (int k = 0; k < n; k++) {
              for (int l = 0; l < n; l++) {
                  int j = sc.nextInt();
